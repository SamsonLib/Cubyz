...
pub const User = struct { // MARK: User
    ...
    fn hexToByte(a: u8, b: u8) !u8 {
        return try std.fmt.parseInt(u8, &[_]u8{ a, b }, 16);
    }
    ...
    fn parse_name(
        ...
        while (i < input.len) {
            if (input[i] == '§' or input[i] == '*' or input[i] == '_') {
                i += 1;
                continue;
            }

            if (i + 6 < input.len and input[i] == '#') {
                const r = std.fmt.parseInt(u8, input[i + 1 .. i + 3], 16) catch {
                    try out.append(allocator, '#');
                    i += 1;
                    continue;
                };
                const g = std.fmt.parseInt(u8, input[i + 3 .. i + 5], 16) catch {
                    try out.append(allocator, '#');
                    i += 1;
                    continue;
                };
                const b = std.fmt.parseInt(u8, input[i + 5 .. i + 7], 16) catch {
                    try out.append(allocator, '#');
                    i += 1;
                    continue;
                };

                const esc_prefix: []const u8 = "\x1b[38;2;";
                try out.appendSlice(allocator, esc_prefix);

                var buf: [16]u8 = undefined;
                const slice = try std.fmt.bufPrint(&buf, "{d};{d};{d}m", .{ r, g, b });
        ...
        const reset_slice: []const u8 = "\x1b[0m";
        try out.appendSlice(allocator, reset_slice);
